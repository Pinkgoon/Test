...
export function tryShoot(state, player, dt){
  // 각 무기가 직접 발사하므로 현재는 미사용
  return false;
}

// ──────────────────────────────────────────────────────────────────────────
// 총알 업데이트/충돌
// ❶ 거리 기반 수명: b.maxDist(px) & b.travel(px)
// ❷ 시간 기반 수명: b.t(sec)
// ❸ 훅 지원: b.onHit(e, state), b.onExpire(state)
export function updateBullets(state, cvs){
  ...
  for(let i=state.bullets.length-1;i>=0;i--){
    ...
    if (b.maxDist!=null){
      ...
      b.travel = (b.travel||0) + Math.hypot(dx,dy);
      if (b.travel >= b.maxDist){
        // 사거리 소진
        state.bullets.splice(i,1);
        if (typeof b.onExpire === 'function') b.onExpire(state, b);
        continue;
      }
    }
    ...
    // 화면 밖 제거(약간 여유) — 일부 무기는 offscreen에선 제거
    ...
    for(let j=state.enemies.length-1;j>=0;j--){
      ...
      if (dx*dx + dy*dy <= rr*rr){
        // onHit 훅(미사일 등)
        if (typeof b.onHit === 'function') b.onHit(e, state, b);
        ...
        const kb = b.kb ?? 110;
        applyKnockback(e, b.x - vx*0.01, b.y - vy*0.01, kb);
